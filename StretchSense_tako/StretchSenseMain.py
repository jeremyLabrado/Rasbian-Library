...
qtCreatorFile = 'StretchSense_tako.ui'
...
class TakoThread(QtCore.QThread):

    def __init__(self):
        super(TakoThread, self).__init__()
    ...
    def run(self):
        stretchsenseObject.ble_waitNotifications()
        self.emit(QtCore.SIGNAL("takoThreadDone()"))
...
class QCustomTakoWidget(QtGui.QWidget):

    def __init__(self, parent=None):
        super(QCustomTakoWidget, self).__init__(parent)
        self.textQVBoxLayout = QtGui.QVBoxLayout()
        self.takoLabel = QtGui.QLabel()
        self.textQVBoxLayout.addWidget(self.takoLabel)
        self.allQHBoxLayout = QtGui.QHBoxLayout()
        self.allQHBoxLayout.addLayout(self.textQVBoxLayout, 0)
        self.setLayout(self.allQHBoxLayout)
        self.myListOfTakoWidget = []

        self.takoLabel.setStyleSheet('''background-color: rgba(0, 0, 0, 0);
                                                border-style: solid;
                                                border-width: 1px;
                                                border-color: transparent;''')

    def setTakoLabel(self, index, value):
        self.takoLabel.setText(str(index) + " : " + str(value))

    def listOfTakoWidget(self, myTakoWidget):
        self.myListOfTakoWidget.append(myTakoWidget)

    def setWidget(self, index, value):
        self.setTakoLabel(index, value)

    def update(self, index, value):
        self.setTakoLabel(str(index), str(value))
        if (value >= 50 and value <= 500):
            self.takoLabel.setStyleSheet('''background-color: rgb(0, 255, 0);
                                             border-color: transparent;''')
        else:
            self.takoLabel.setStyleSheet('''background-color: rgba(255, 0, 0, 160);
                                         border-color: transparent;''')
...
class StretchSenseApp(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.takoRecordingButton = True
        self.spiDataRecorded = ""
        self.dataRecorded = ""
        self.takoDataRecorded = ""
        self.myCustomQWidget = QCustomQWidget()
        self.myTakoQWidget = QCustomTakoWidget()
        self.listOfCustomWidget = []
        self.myListOfScanWidget = []
        self.myListOfTakoWidget = []
        self.listOfTakoWidget = []
        self.t0 = time.time()
        self.counter = 0
        self.myAddrToCheck = ""
        self.stateValue = ""
        ...
        self.today = ""
        self.IDName = ""
        self.w_idNameLabel.setText("Select State and Pattern")

        self.SpiThread = SpiThread()
        self.BleThread = BleThread()
        self.TakoThread = TakoThread()
        ...
        self.connect(self.TakoThread, QtCore.SIGNAL("takoThreadDone()"), self.takoThreadDone, QtCore.Qt.DirectConnection)
        ...
        self.w_backArrow9.clicked.connect(self.bleTab)
        ...
        self.w_takoTableButton.clicked.connect(self.bleTakoTab)
        ...
        self.w_takoRecordButton.clicked.connect(self.takoRecordButton)
        ...
        self.w_stateList.clicked.connect(self.bleNameId)
        self.w_patternList.clicked.connect(self.bleNameId)
        ...
        self.w_stateList.addItem("LAYER 6")
        self.w_stateList.addItem("LAYER 4")
        self.w_stateList.addItem("FABRIC")
        self.w_stateList.addItem("PCB ADHESION")

        self.w_patternList.addItem("LEFT")
        self.w_patternList.addItem("RIGHT")
        self.w_patternList.addItem("FRONT")
        self.w_patternList.addItem("BACK")
    ...
    def takoThreadDone(self):
        #print("takoThreadDone()")

        """

        Action triggered by the Tako thread that updates our values

        """

        for i in range(len(self.listOfTakoWidget)):
            if i == 77:
                pass
            else:
                self.listOfTakoWidget[i].update(self.listPeripheralConnected[i].channelNumber, self.listPeripheralConnected[i].value)

        self.t1 = time.time()
        self.t1 -= self.t0
        self.t1 = str(self.t1) + " ,"
        self.takoDataRecorded += str(self.counter) + " ," + str(self.t1)
        self.takoDataRecorded += stretchsenseObject.ble_getValuesCsv() + "\n"
        self.counter += 1
        if self.t1[0:2] == "2.":
            self.takoStopRecordData(self.takoDataRecorded)
    ...
    def takoRecordButton(self):
        #print("bleRecordButton()")

        """
        Check if the Tako record button has been pushed.

        """
        if self.w_idNameLabel.palette().color(QtGui.QPalette.Background).name() != "#ff0000":
            if self.takoRecordingButton is True:
                self.takoRecordData()
            elif self.takoRecordingButton is False:
                self.takoStopRecordData(self.takoDataRecorded)
        else:
            pass

    def takoRecordData(self):
        #print("bleRecordData()")
        ...
        self.w_takoRecordButton.setText("Wait")
        self.takoDataRecorded = ""
        self.takoDataRecorded = str(self.stateValue) + "\n" + str(self.patternValue) + "\n" + str(self.today) + "\n\n" + "#, Sample Time         , Samples" + "\n"
        self.counter = 0
        self.t0 = time.time()
        self.takoRecordingButton = False
        #self.takoStopRecordData(self.takoDataRecorded)

    def takoStopRecordData(self, data):
        #print("bleStopRecordData()")
        ...
        if self.takoRecordingButton is False:
            self.w_takoRecordButton.setText("Save")
            filename = self.w_directoryName.text() + "/" + self.IDName
            if filename != 0:
                self.recordData(data, filename)
                self.takoRecordingButton = True
                #self.averageTakoValues(filename)
            else:
                self.w_generalTab.setCurrentIndex(self.indexBle)
    ...
    def bleNameId(self):
        ...
        if self.w_stateList.currentItem():
            self.stateValue = self.w_stateList.currentItem().text()
        if self.w_patternList.currentItem():
            self.patternValue = self.w_patternList.currentItem().text()
        if ((self.patternValue != "") and (self.stateValue != "")):
            print("if")

            self.today = datetime.datetime.now().strftime("%y-%m-%d-%H-%M-%S")
            self.IDName = str(self.stateValue) + "_" + str(self.patternValue) + "_" + str(self.today)
            self.w_idNameLabel.setText(self.IDName)
        if self.w_idNameLabel.text()[0:5] != "Select":
            self.w_idNameLabel.setStyleSheet("""background-color: rgb(0, 255, 0);
                                                font: rgb(0, 0, 0)""")

    def bleTakoValueTable(self):
        #print("bleTakoValueTable()")

        stretchsenseObject.ble_waitNotifications()
        self.listPeripheralConnected = stretchsenseObject.ble_getListPeripheralIsConnected()
        numberOfPeripheralConnected = len(self.listPeripheralConnected)
        self.myListOfTakoWidget.clear()

        if numberOfPeripheralConnected > 0:

            self.TakoThread.start()
            self.listOfTakoWidget.clear()
            for myPeripheral in self.listPeripheralConnected:
                self.myTakoQWidget = QCustomTakoWidget()
                index = int(myPeripheral.channelNumber)
                value = myPeripheral.value
                row = index % 16
                column = index // 16
                self.myTakoQWidget.setWidget(index, value)
                self.listOfTakoWidget.append(self.myTakoQWidget)

                # Add QListWidgetItem into QListWidget
                self.w_listDataPrintedTako.setCellWidget(row, column, self.myTakoQWidget)
    ...
    def mainTab(self):
        ...
        if self.BleThread != 0:
            self.BleThread.stop()
        if self.TakoThread != 0:
            self.TakoThread.stop()
        else:
            pass
    ...
    def bleTakoTab(self):
        ...
        self.w_generalTab.setCurrentIndex(self.indexTako)
        self.bleTakoValueTable()
    ...
    def averageTakoValues(self, filename):
        print("averageTakoValues()")
